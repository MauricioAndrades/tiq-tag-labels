(function() {
    ...
    tag.elems.map(function(elem) {
        elem.dataset.loadRules = utui.data.manage[elem.dataset.id].loadrule;
        if (tag.scope[elem.dataset.id]) {
            elem.dataset.extensions = tag.scope[elem.dataset.id].join(', ');
        } else {
            elem.dataset.extensions = "";
        }
        return elem;
    });
    // build mapping container elems
    tag.elems.forEach(function(elem) {

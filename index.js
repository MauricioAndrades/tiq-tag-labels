function extra_info() {
	return (function() {
			...
			csm.create_checkbox = {
				tags: function() {
					...
					var chkboxes = {
						...
						// labels: '<div class="tab-menu-item"><input class="chkbox_manage" id="labels_chkbox" type="checkbox" value="labels"><label for="labels_chkbox"><i class="icon-folder-close mapping-icon"></i></label></div>'
					}
					node.appendChild(csm.parseHTML(chkboxes.lr)[0]);
					node.appendChild(csm.parseHTML(chkboxes.ext)[0]);
				// node.appendChild(csm.parseHTML(chkboxes.labels)[0]);
				var loadrules_chkbox = document.querySelector('#loadrules_chkbox');
				var extensions_chkbox = document.querySelector('#ext_chkbox');
				// var labels_chkbox = document.querySelector('#labels_chkbox');
		...
		csm.ExtensionsInfoContainer = class ExtensionsInfoContainer {
			constructor(scope) {
				if (document.querySelector('#loadrules_chkbox') === null) csm.create_checkbox.customizations();
		...
		csm.add_restore_callback = function() {
			$(".tabLabel").on('mouseup', function(e) {
				...
				window.requestIdleCallback(function() {
					csm.get_checkbox_state() ? csm.labels.hide() : csm.labels.show()
					if (csm.has_info()) {
						csm.remove_info();
					}
					csm.add_info();
